...
const SimpleCheckoutModal: React.FC<SimpleCheckoutModalProps> = ({
  ...
  const createStripeOrder = async () => {
    ...
    try {
      ...
      console.log('🆔 Creating Stripe session for order with client ID:', clientIdentity.clientId.slice(-12));

      // 🔒 CRITICAL: DO NOT CREATE ORDER YET!
      // Order will be created by Stripe webhook AFTER payment confirmation
      // Prepare order data to send in Stripe session metadata
      ...
      console.log('📦 Order data prepared for Stripe session metadata (order will be created AFTER payment)');
      ...
      // Prepare Stripe checkout session data with ALL order data in metadata
      ...
      const stripeData = {
        ...
        metadata: {
          // 🎯 ALL ORDER DATA - webhook will create order from this
          ...
          delivery_fee: (addressValidation?.deliveryFee || 0).toString(),
          ...
          isAuthenticatedOrder: isAuthenticated.toString(),
          coordinates: JSON.stringify(addressValidation?.coordinates || {}),
          formattedAddress: addressValidation?.formattedAddress || '',
          order_items: JSON.stringify(orderItemsData), // All items with extras
          source: 'francesco_fiori_website',
          order_type: 'cart_order',
        }
      };

      console.log('📤 Creating Stripe checkout session (order will be created AFTER payment)...');
      console.log('📋 Order data in Stripe metadata:', stripeData.metadata);
      ...
      console.log('🔒 Order NOT created yet - will be created by webhook after payment confirmation');
      console.log(`📋 Order number ${orderNumber} ready for creation after payment`);
