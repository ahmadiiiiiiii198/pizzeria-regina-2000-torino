/**
 * STRIPE WEBHOOK V2 - Simplified for debugging
 */
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (settingError || !setting) {
      console.error('❌ Webhook secret not found:', settingError);
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'Webhook secret not configured', details: settingError }),
      };
    }
    ...
    if (stripeEvent.type === 'checkout.session.completed') {
      ...
      if (!metadata || !metadata.order_items) {
        ...
        return { 
          ...
          body: JSON.stringify({ 
            received: true, 
            message: 'No order data in metadata' 
          }) 
        };
      }
      ...
      console.log('Creating order:', metadata.order_number);
      ...
      if (orderError) {
        ...
        return {
          statusCode: 500,
          body: JSON.stringify({ error: 'Order creation failed', details: orderError }),
        };
      }
